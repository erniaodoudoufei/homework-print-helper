...
![图片处理和原图对比](app/docs/screenshots/editor.png)

![A4 打印预览](app/docs/screenshots/print-preview.png)

## 适用范围

适合文字、表格和手写资料。纸边缺失或自动识别不可靠时保留范围，需手动调整四角。浅色虚线和彩色笔迹建议放大检查，必要时降低去灰底强度。

第一版不包含 OCR、笔迹擦除、PDF 导入、扫描仪采集、拼页或弯曲书页展平；不能补出原照片未记录的内容。

## 本地开发

源码位于 `app` 子目录。使用 Python 3.12 x64，从仓库根目录运行：
...
处理顺序为 EXIF → 四角透视 → 90° 旋转 → 小角度纠偏 → 矩形裁切 → 去底色 → 笔迹深浅 → 可选灰度。预览与最终输出都从原图重新计算；历史记录只存参数。
...
软件测试已通过 20 项，并验证便携版在独立目录、移除 Python 开发环境变量后仍能运行。实体打印效果尚未实测，需结合纸张、墨水和打印驱动实际检查。
...
第三方组件及许可见 [THIRD_PARTY_NOTICES.md](app/THIRD_PARTY_NOTICES.md)，便携程序附带组件许可文本。
